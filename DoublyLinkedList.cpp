...
public:
    ...
    void insertAtPosition(int data, int position) {
        ...
        shared_ptr<Node> newNode = make_shared<Node>(data); // Create a new node with the given data
        auto current = head; // Start from the head of the list
        while (current && currentPosition < position - 1) { // Traverse the list to find the position
            current = current->next; // Move to the next node
            currentPosition++; // Increment the current position
        }

        if (!current) { // If current is null, it means position is greater than the size of the list
    ...
    void deleteFromBeginning() {
        ...
        if (!head) { // If the list is empty, nothing to delete
    ...
    void traverseForward() {
        // This function traverses the list from head to tail
        cout << "Forward traversal function is not implemented yet." << endl;
    };

    void traverseBackward() {
        // This function traverses the list from tail to head
        cout << "Backward traversal function is not implemented yet." << endl;
    };

    void reverse() {
        // This function reverses the doubly linked list
        cout << "Reverse function is not implemented yet." << endl;
    };
